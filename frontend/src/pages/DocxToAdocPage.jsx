...
import { FileText, Upload, CheckCircle2, Loader2, XCircle, Download } from 'lucide-react'
...
export default function DocxToAdocPage() {
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    setFiles(docxFiles)
    setError(null)
    addLog(`Selected ${docxFiles.length} DOCX file(s) for conversion`, 'info')
  }, [addLog])
  ...
  const handleConvert = async () => {
    if (files.length === 0) {
      setError('Please select at least one DOCX file')
    ...
    addLog(`Starting DOCX to AsciiDoc conversion of ${files.length} file(s)...`, 'info')
    ...
    try {
      ...
      setProcessingStatus('Converting files with Pandoc...')

      const result = await window.electronAPI.convertDocxToAdoc(fileData, { docType })
      ...
      if (result.success) {
        ...
        addLog(`Successfully converted ${successCount} file(s) to AsciiDoc`, 'success')
        setProcessingStatus(`Complete! ${successCount} succeeded, ${failCount} failed`)
      } else {
        setError('Conversion failed. Please try again.')
        addLog(`DOCX to AsciiDoc conversion failed`, 'error')
      }

    } catch (err) {
      const errorMsg = err.message || 'Conversion failed. Please try again.'
      setError(errorMsg)
      addLog(`DOCX to AsciiDoc conversion failed: ${errorMsg}`, 'error')
      setProcessingStatus('')
      setFileStatuses({})
    } finally {
      setLoading(false)
  ...
  const handleDownload = async () => {
    ...
    link.href = url;
    link.download = `converted_adoc_${Date.now()}.zip`;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-3xl font-bold">DOCX to S1000D AsciiDoc</h2>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Convert Word documents to S1000D AsciiDoc format using Pandoc.
        ...
        <CardContent className="space-y-6">
          ...
          <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
            ...
            <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
              <li>Pandoc is bundled with the application</li>
              <li>DOCX files should follow DMC naming convention</li>
          ...
          <Button
            ...
            className="w-full"
          >
            {loading ? 'Converting...' : `Convert ${files.length || ''} DOCX to AsciiDoc`}
          </Button>
