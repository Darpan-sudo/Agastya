...
export default function PdfToDocxPage() {
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    setFiles(pdfFiles)
    setError(null)
    addLog(`Selected ${pdfFiles.length} PDF file(s) for conversion`, 'info')
  }, [addLog])
  ...
  const handleConvert = async () => {
    ...
    addLog(`Starting PDF to DOCX conversion of ${files.length} file(s)...`, 'info')
    ...
    try {
      ...
      setProcessingStatus('Converting files with pdf2docx...')

      const result = await window.electronAPI.convertPdfToDocx(fileData)
      ...
      if (result.success) {
        ...
        addLog(`Successfully converted ${successCount} file(s) to DOCX`, 'success')
        setProcessingStatus(`Complete! ${successCount} succeeded, ${failCount} failed`)
      } else {
        setError('Conversion failed. Please try again.')
        addLog(`PDF to DOCX conversion failed`, 'error')
      }

    } catch (err) {
      const errorMsg = err.message || 'Conversion failed. Please try again.'
      setError(errorMsg)
      addLog(`PDF to DOCX conversion failed: ${errorMsg}`, 'error')
      setProcessingStatus('')
      setFileStatuses({})
    } finally {
      setLoading(false)
  ...
  const handleDownload = async () => {
    ...
    link.href = url;
    link.download = `converted_docx_${Date.now()}.zip`;
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Convert PDF documents to editable DOCX format using pdf2docx.
        ...
        <CardContent className="space-y-6">
          <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
            ...
            <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
              <li>Python and pdf2docx library are bundled</li>
          ...
          <Button
            ...
            className="w-full"
          >
            {loading ? 'Converting...' : `Convert ${files.length || ''} PDF to DOCX`}
          </Button>
